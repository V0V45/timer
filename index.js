...
function timerCheckAndLaunch(event) { // функция запуска таймера
    ...
    if (timerMain) {
        ...
        clearInterval(timerAdditional); // и дополнительный таймер тоже вырубаем
    }
    ...
    timerAdditional = setInterval(updateAdditionalTimer, 60000); // запускаем дополнительный таймер, который выводит алерт каждую минуту
}
...
function updateTimer() { // основной таймер
    ...
    if (minutes === 0 && seconds === 0) { // когда отсчет дойдет до конца
        ...
        clearInterval(timerAdditional); // выключаем дополнительный таймер
...
function updateAdditionalTimer() { // дополнительный таймер
    console.log('Прошла минута'); // если прошла минута - выводим сообщение
}
